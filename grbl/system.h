...
#include "grbl.h"
/// \brief  Define system executor bit map. Used internally by realtime protocol as realtime command flags,
...
/// \brief  Alarm executor codes. Valid values (1-255). Zero is reserved.
...
/// \brief  Override bit maps. Realtime bitflags to control feed, rapid, spindle, and coolant overrides.
...
/// \brief  Define system state bit map. The state variable primarily tracks the individual functions
...
/// \brief  Define system suspend flags. Used in various ways to manage suspend states and procedures.
...
/// \brief  Define control pin index for Grbl internal use. Pin maps may change, but these values don't.
#define N_CONTROL_PIN 4
#define CONTROL_PIN_INDEX_SAFETY_DOOR   bit(0)
#define CONTROL_PIN_INDEX_RESET         bit(1)
#define CONTROL_PIN_INDEX_FEED_HOLD     bit(2)
#define CONTROL_PIN_INDEX_CYCLE_START   bit(3)
/// \brief  Define spindle stop override control states.
...
/// \brief  Define global system variables
typedef struct {
...
void system_init();
/// \brief  Returns bitfield of control pin states, organized by CONTROL_PIN_INDEX. (1=triggered, 0=not triggered).
uint8_t system_control_get_state();
/// \brief  Returns if safety door is open or closed, based on pin state.
uint8_t system_check_safety_door_ajar();
/// \brief  Executes an internal system command, defined as a string starting with a '$'
uint8_t system_execute_line(char *line);
/// \brief  Execute the startup script lines stored in EEPROM upon initialization
...
/// \brief  Updates a machine 'position' array based on the 'step' array sent.
void system_convert_array_steps_to_mpos(float *position, int32_t *steps);
/// \brief  CoreXY calculation only. Returns x or y-axis "steps" based on CoreXY motor steps.
#ifdef COREXY
...
/// \brief  Special handlers for setting and clearing Grbl's real-time execution flags.
