...
/*! \brief  Define Adaptive Multi-Axis Step-Smoothing(AMASS) levels and cutoff frequencies. The highest level
  ...
  \note AMASS cutoff frequency multiplied by ISR overdrive factor must not exceed maximum step frequency.
  
  \note Current settings are set to overdrive the ISR to no more than 16kHz, balancing CPU overhead
...
/// \brief  Stores the planner block Bresenham algorithm execution data for the segments in the segment
...
/// \brief  Primary stepper segment ring buffer. Contains small, short line segments for the stepper
...
/// \brief  Stepper ISR data struct. Contains the running data for the main stepper ISR.
typedef struct {
...
static stepper_t st;
/// \brief  Step segment ring buffer indices
static volatile uint8_t segment_buffer_tail;
static uint8_t segment_buffer_head;
static uint8_t segment_next_head;
/// \brief  Step and direction port invert masks.
static uint8_t step_port_invert_mask;
static uint8_t dir_port_invert_mask;
/// \brief  Used to avoid ISR nesting of the "Stepper Driver Interrupt". Should never occur though.
static volatile uint8_t busy;
/// \brief  Pointers for the step segment being prepped from the planner buffer. Accessed only by the
// main program. Pointers may be planning segments or planner blocks ahead of what being executed.
static plan_block_t *pl_block;     //!< Pointer to the planner block being prepped
static st_block_t *st_prep_block;  //!< Pointer to the stepper block data being prepped
/// \brief  Segment preparation data struct. Contains all the necessary information to compute new segments
...
/*! \brief    BLOCK VELOCITY PROFILE DEFINITION
...
/// \brief  Stepper state initialization. Cycle should only start if the st.cycle_start flag is
...
/*! \brief "The Stepper Driver Interrupt" - This timer interrupt is the workhorse of Grbl. 

   Grbl employs the venerable Bresenham line algorithm to manage and exactly synchronize 
   multi-axis moves. Unlike the popular DDA algorithm, the Bresenham algorithm is not susceptible 
   to numerical round-off errors and only requires fast integer counters, meaning low computational
   overhead and maximizing the Arduino's capabilities. 
   
   However, the downside of the Bresenham algorithm is, for certain multi-axis motions, the 
   non-dominant axes may suffer from un-smooth step pulse trains, or aliasing, which can lead to
   strange audible noises or shaking. This is particularly noticeable or may cause motion issues
   at low step frequencies (0-5kHz), but is usually not a physical problem at higher frequencies,
   although audible.
   ...
   step count for each AMASS level. 
   
   For example, for a Level 1 step smoothing, we bit shift the Bresenham step event count,
   effectively multiplying it by 2, while the axis step counts remain the same, and then double
   the stepper ISR frequency. In effect, we are allowing the non-dominant Bresenham axes step in 
   the intermediate ISR tick, while the dominant axis is stepping every two ISR ticks, rather 
   than every ISR tick in the traditional sense. 
   
   At AMASS Level 2, we simply bit-shift again, so the non-dominant Bresenham axes can step within
   any of the four ISR ticks, the dominant axis steps every four ISR ticks, and quadruple the
   ...
   \note This interrupt must be as efficient as possible and complete before the next ISR tick,
   which for Grbl must be less than 33.3usec (@30kHz ISR rate). Oscilloscope measured time in
   ISR is 5usec typical and 25usec maximum, well below requirement.
   
   \note This ISR expects at least one step to be executed per segment.

   \todo  Replace direct updating of the int32 position counters in the ISR somehow. Perhaps use smaller
...
ISR(TIMER1_COMPA_vect)
{
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      st.step_count = st.exec_segment->n_step; //!< \note Can sometimes be zero when moving slow.
...
/*! \brief The Stepper Port Reset Interrupt: 
   Timer0 OVF interrupt handles the falling edge of the step
   ...
   \note Interrupt collisions between the serial and stepper interrupts can cause delays by
   a few microseconds, if they execute right before one another. Not a big deal, but can
   cause issues at high step rates if another high frequency asynchronous interrupt is
   added to Grbl.
   
   \note This interrupt is enabled by ISR_TIMER1_COMPAREA when it sets the motor port bits to execute
...
#ifdef STEP_PULSE_DELAY
  /*! \brief This interrupt is used only when STEP_PULSE_DELAY is enabled. 
  Here, the step pulse is initiated after the STEP_PULSE_DELAY time period has elapsed. The 
  ISR TIMER2_OVF interrupt will then trigger after the appropriate settings.pulse_microseconds, 
  as in normal operation. The new timing between direction, step pulse, and step complete events
  are setup in the st_wake_up() routine.
  */
...
/*! \brief Prepares step segment buffer. Continuously called from main program.
   ...
   \note Computation units are in steps, millimeters, and minutes.
*/
void st_prep_buffer()
{
...
//! \todo    Determine correct handling of parameters in deceleration-only.
