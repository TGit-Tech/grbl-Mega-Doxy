...
/// \brief  Initializes coolant control pins.
void coolant_init();
/// \brief  Returns current coolant output state. Overrides may alter it from programmed state.
uint8_t coolant_get_state();
/// \brief  Immediately disables coolant pins.
void coolant_stop();
/// \brief  Sets the coolant pins according to state specified.
void coolant_set_state(uint8_t mode);
/// \brief  G-code parser entry-point for setting coolant states. Checks for and executes additional conditions.
