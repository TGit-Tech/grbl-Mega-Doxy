...
*/
/// \brief  This file contains compile-time configurations for Grbl's internal system. For the most part,
// users will not need to directly modify these, but they are here for specific needs, i.e.
// performance tuning or adjusting to non-typical machines.
/// \brief  IMPORTANT: Any changes here requires a full re-compiling of the source code to propagate them.

#ifndef config_h
#define config_h
#include "grbl.h" //!< For Arduino IDE compatibility.

/// \brief  Define CPU pin map and default settings.
// 
...
/// \brief  Define realtime command special characters. These characters are 'picked-off' directly from the
...
/// \brief  If homing is enabled, homing init lock sets Grbl into an alarm state upon power up. This forces
// the user to perform the homing cycle (or override the locks) before doing anything else. This is
// mainly a safety feature to remind the user to home, since position is unknown to Grbl.
#define HOMING_INIT_LOCK //!< Comment to disable
/// \brief  Define the homing cycle patterns with bitmasks. The homing cycle first performs a search mode
...
/// \brief  #define HOMING_CYCLE_0 (1<<X_AXIS)  //!< COREXY COMPATIBLE: First home X
// #define HOMING_CYCLE_1 (1<<Y_AXIS)  //!< COREXY COMPATIBLE: Then home Y
/// \brief  Number of homing cycles performed after when the machine initially jogs to limit switches.
// This help in preventing overshoot and should improve repeatability. This value should be one or
// greater.
#define N_HOMING_LOCATE_CYCLE 1 //!< Integer (1-128)
/// \brief  Enables single axis homing commands. $HX, $HY, and $HZ for X, Y, and Z-axis homing. The full homing 
// cycle is still invoked by the $H command. This is disabled by default. It's here only to address
// users that need to switch between a two-axis and three-axis machine. This is actually very rare.
// If you have a two-axis machine, DON'T USE THIS. Instead, just alter the homing cycle for two-axes.
// #define HOMING_SINGLE_AXIS_COMMANDS //!< Default disabled. Uncomment to enable.
/// \brief  After homing, Grbl will set by default the entire machine space into negative space, as is typical
// for professional CNC machines, regardless of where the limit switches are located. Uncomment this
// define to force Grbl to always set the machine origin at the homed location despite switch orientation.
// #define HOMING_FORCE_SET_ORIGIN //!< Uncomment to enable.
/// \brief  Number of blocks Grbl executes upon startup. These blocks are stored in EEPROM, where the size
// and addresses are defined in settings.h. With the current settings, up to 2 startup blocks may
// be stored and executed in order. These startup blocks would typically be used to set the g-code
// parser state depending on user preferences.
#define N_STARTUP_LINE 2 //!< Integer (1-2)
/// \brief  Number of floating decimal points printed by Grbl for certain value types. These settings are
...
/// \brief  If your machine has two limits switches wired in parallel to one axis, you will need to enable
...
/// \brief  Upon a successful probe cycle, this option provides immediately feedback of the probe coordinates
// through an automatically generated message. If disabled, users can still access the last probe
// coordinates through Grbl '$#' print parameters.
#define MESSAGE_PROBE_COORDINATES //!< Enabled by default. Comment to disable.
/// \brief  This option causes the feed hold input to act as a safety door switch. A safety door, when triggered,
// immediately forces a feed hold and then safely de-energizes the machine. Resuming is blocked until
// the safety door is re-engaged. When it is, Grbl will re-energize the machine and then resume on the
// previous tool path, as if nothing happened.
// #define ENABLE_SAFETY_DOOR_INPUT_PIN //!< Default disabled. Uncomment to enable.
/// \brief  After the safety door switch has been toggled and restored, this setting sets the power-up delay
...
/// \brief  Inverts pin logic of the control command pins based on a mask. This essentially means you can use
...
/// \brief  Inverts select limit pin states based on the following mask. This effects all limit pin functions,
...
/// \brief  Inverts the spindle enable pin from low-disabled/high-enabled to low-enabled/high-disabled. Useful
// for some pre-built electronic boards.
// #define INVERT_SPINDLE_ENABLE_PIN //!< Default disabled. Uncomment to enable.
/// \brief  Inverts the selected coolant pin from low-disabled/high-enabled to low-enabled/high-disabled. Useful
// for some pre-built electronic boards.
// #define INVERT_COOLANT_FLOOD_PIN //!< Default disabled. Uncomment to enable.
// #define INVERT_COOLANT_MIST_PIN //!< Default disabled. Note: Enable M7 mist coolant in config.h
/// \brief  When Grbl powers-cycles or is hard reset with the Arduino reset button, Grbl boots up with no ALARM
...
/// \brief  At power-up or a reset, Grbl will check the limit switch states to ensure they are not active
...
/// \brief  Enables code for debugging purposes. Not for general use and always in constant flux.
// #define DEBUG //!< Uncomment to enable. Default disabled.
/// \brief  Configure rapid, feed, and spindle override settings. These values define the max and min
...
/// \brief  When a M2 or M30 program end command is executed, most g-code states are restored to their defaults.
// This compile-time option includes the restoring of the feed, rapid, and spindle speed override values
// to their default values at program end.
#define RESTORE_OVERRIDES_AFTER_PROGRAM_END //!< Default enabled. Comment to disable.
/// \brief  The status report change for Grbl v1.1 and after also removed the ability to disable/enable most data
...
/// \brief  Some status report data isn't necessary for realtime, only intermittently, because the values don't
...
/// \brief  The temporal resolution of the acceleration management subsystem. A higher number gives smoother
...
/// \brief  Adaptive Multi-Axis Step Smoothing (AMASS) is an advanced feature that does what its name implies,
// smoothing the stepping of multi-axis motions. This feature smooths motion particularly at low step
// frequencies below 10kHz, where the aliasing between axes of multi-axis motions can cause audible
// noise and shake your machine. At even lower step frequencies, AMASS adapts and provides even better
// step smoothing. See stepper.c for more details on the AMASS system works.
#define ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING  //!< Default enabled. Comment to disable.
/// \brief  Sets the maximum step rate allowed to be written as a Grbl setting. This option enables an error
...
/// \brief  By default, Grbl sets all input pins to normal-high operation with their internal pull-up resistors
...
/// \brief  Sets which axis the tool length offset is applied. Assumes the spindle is always parallel with
// the selected axis with the tool oriented toward the negative direction. In other words, a positive
// tool length offset value is subtracted from the current location.
#define TOOL_LENGTH_OFFSET_AXIS Z_AXIS //!< Default z-axis. Valid values are X_AXIS, Y_AXIS, or Z_AXIS.
/// \brief  Used by variable spindle output only. This forces the PWM output to a minimum duty cycle when enabled.
...
/// \brief  With this enabled, Grbl sends back an echo of the line it has received, which has been pre-parsed (spaces
...
/// \brief  Minimum planner junction speed. Sets the default minimum junction speed the planner plans to at
...
/// \brief  Sets the minimum feed rate the planner will allow. Any value below it will be set to this minimum
// value. This also ensures that a planned motion always completes and accounts for any floating-point
// round-off errors. Although not recommended, a lower value than 1.0 mm/min will likely work in smaller
// machines, perhaps to 0.1mm/min, but your success may vary based on multiple factors.
#define MINIMUM_FEED_RATE 1.0 //!< (mm/min)
/// \brief  Number of arc generation iterations by small angle approximation before exact arc trajectory
// correction with expensive sin() and cos() calcualtions. This parameter maybe decreased if there
// are issues with the accuracy of the arc generations, or increased if arc execution is getting
// bogged down by too many trig calculations.
#define N_ARC_CORRECTION 12 //!< Integer (1-255)
/// \brief  The arc G2/3 g-code standard is problematic by definition. Radius-based arcs have horrible numerical
...
/// \brief  Time delay increments performed during a dwell. The default value is set at 50ms, which provides
// a maximum time delay of roughly 55 minutes, more than enough for most any application. Increasing
// this delay will increase the maximum dwell time linearly, but also reduces the responsiveness of
// run-time command executions, like status reports, since these are performed between each dwell
// time step. Also, keep in mind that the Arduino delay timer is not very accurate for long delays.
#define DWELL_TIME_STEP 50 //!< Integer (1-255) (milliseconds)
/// \brief  Creates a delay between the direction pin setting and corresponding step pulse by creating
...
/// \brief  The number of linear motions in the planner buffer to be planned at any give time. The vast
...
/// \brief  Governs the size of the intermediary step segment buffer between the step execution algorithm
...
/// \brief  Line buffer size from the serial input stream to be executed. Also, governs the size of
...
/// \brief  The maximum line length of a data string stored in EEPROM. Used by startup lines and build
...
/// \brief  A simple software debouncing feature for hard limit switches. When enabled, the interrupt 
...
/// \brief  Configures the position after a probing cycle during Grbl's check mode. Disabled sets
// the position to the probe target, when enabled sets the position to the start position.
// #define SET_CHECK_MODE_PROBE_TO_START //!< Default disabled. Uncomment to enable.
/// \brief  Force Grbl to check the state of the hard limit switches when the processor detects a pin
...
/// \brief  Adjusts homing cycle search and locate scalars. These are the multipliers used by Grbl's
...
/// \brief  Enable the '$RST=*', '$RST=$', and '$RST=#' eeprom restore commands. There are cases where
...
/// \brief  Defines the EEPROM data restored upon a settings version change and `$RST=*` command. Whenever the
...
/// \brief  Enable the '$I=(string)' build info write command. If disabled, any existing build info data must
...
/// \brief  AVR processors require all interrupts to be disabled during an EEPROM write. This includes both
...
/// \brief  In Grbl v0.9 and prior, there is an old outstanding bug where the `WPos:` work position reported
...
/// \brief  By default, Grbl disables feed rate overrides for all G38.x probe cycle commands. Although this
// may be different than some pro-class machine control, it's arguable that it should be this way. 
// Most probe sensors produce different levels of error that is dependent on rate of speed. By 
// keeping probing cycles to their programmed feed rates, the probe sensor should be a lot more
// repeatable. If needed, you can disable this behavior by uncommenting the define below.
// #define ALLOW_FEED_OVERRIDE_DURING_PROBE_CYCLES //!< Default disabled. Uncomment to enable.
/// \brief  Enables and configures parking motion methods upon a safety door state. Primarily for OEMs
...
/// \brief  Configure options for the parking motion, if enabled.
...
/// \brief  Enables and configures Grbl's sleep mode feature. If the spindle or coolant are powered and Grbl 
...
/// \brief  This option will automatically disable the laser during a feed hold by invoking a spindle stop
...
/*! \brief ---------------------------------------------------------------------------------------
   ...
   below.
*/
/// \brief  Paste CPU_MAP definitions here.
/// \brief  Paste default settings definitions here.
